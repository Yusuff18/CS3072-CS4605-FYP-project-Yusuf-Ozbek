document.addEventListener('DOMContentLoaded', () => {
  ...
  async function init(){
    const [settings, ledger, origin] = await Promise.all([getSettings(), getLedger(), getCurrentOrigin()]);
    currentOrigin = origin;
    if (els.siteName) els.siteName.textContent = origin ? `This site: ${origin}` : `This site: unknown`;

    // reflect global settings
  ...
  function updateModeHint(){
    ...
    if (m === 'strict')
      els.modeHint.textContent = "Prefers reject cookies";
    else if (m === 'balanced')
      els.modeHint.textContent = "Rejects if possible, else accept";
    else
      els.modeHint.textContent = "Prefers accept cookies";
  }
  ...
  function renderSites(){
    ...
    for (const it of showing){
      const row  = document.createElement('div'); row.className = 'site-row';
      const left = document.createElement('div'); left.className='site-left';
      ...
      const url  = document.createElement('div'); url.className='site-url';
  ...
  function renderInsights(ledger){
    ...
    const siteTop = Object.entries(siteCounts).sort((a,b)=>b[1].count - a[1].count).slice(0,5);

    // time saved estimate (~3s per auto)
    ...
    els.insightsBody.innerHTML = `
      ...
      <div class="ins-card">
        <h4>Top CMPs</h4>
        <ul class="ul-plain">
          ${cmpTop.length ? cmpTop.map(([k,v]) => `<li>${k} <span class="k">(${v})</span></li>`).join('') : '<li>None</li>'}
        </ul>
      </div>
      ...
      <div class="ins-card">
        ...
        <ul class="ul-plain">
          ${siteTop.length ? siteTop.map(([s,info]) => `<li>${humanizeSite(s)} <span class="k">(${info.count})</span></li>`).join('') : '<li>None</li>'}
        </ul>
      </div>
  ...
  // ---------- Messaging ----------
  function getLedger(){ return new Promise(res => chrome.runtime.sendMessage({ type:'get_ledger' }, r => res(r?.list || []))); }
  function getSettings(){ return new Promise(res => chrome.runtime.sendMessage({ type:'get_settings' }, r => res(r?.settings || {}))); }
  function setSettings(patch){ return new Promise(res => chrome.runtime.sendMessage({ type:'set_settings', patch }, r => res(r?.settings))); }
  ...
  function niceCMP(id){
    const map = {
      onetrust:'OneTrust', sourcepoint:'Sourcepoint', cookiebot:'Cookiebot',
      quantcast:'Quantcast', trustarc:'TrustArc', didomi:'Didomi',
      iubenda:'Iubenda', fides:'Fides', consentmanager:'ConsentManager',
      cookieyes:'CookieYes', osano:'Osano', klaro:'Klaro', axeptio:'Axeptio'
    };
  ...
  function topPairs(obj, n){ return Object.entries(obj).sort((a,b)=>b[1]-a[1]).slice(0, n); }
